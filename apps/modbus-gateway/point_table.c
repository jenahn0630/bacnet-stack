    ...
    GW_CONFIG *cfg, GW_POINT_TABLE *table, const char *filename)
{
    ...
    if (!root) {
        fprintf(stderr, "[PT] Using compile-time defaults (no JSON)\n");
        return false;
    }
    ...
    if (sec) {
        ...
        if (cJSON_IsNumber(j)) {
            cfg->device_instance = (uint32_t)j->valuedouble;
        }
        ...
        if (cJSON_IsNumber(j)) {
            cfg->mstp_mac = (uint8_t)j->valuedouble;
        }
    ...
    cJSON_ArrayForEach(item, points_arr)
    {
        ...
        /* enabled: false 이면 이 포인트는 완전히 건너뜀 */
        ...
        p->modbus_slave = cJSON_IsNumber(j) ? (uint8_t)j->valuedouble : 1;
        ...
        switch (p->bacnet_type) {
            ...
            case GW_BACNET_TYPE_BI:
                Binary_Input_Present_Value_Set(
                    p->bacnet_instance,
                    (fabsf(eng_val) > 0.5f) ? BINARY_ACTIVE : BINARY_INACTIVE);
            ...
            case GW_BACNET_TYPE_AO:
            case GW_BACNET_TYPE_BO:
                /* Read-back monitoring only; write is handled externally */
                break;
            default:
                break;
        }
